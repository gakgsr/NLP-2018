...
class Batch(object):
    ...
    def __init__(self, context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, context_char_ids, qn_char_ids, uuids=None):
        """
        Inputs:
...
def char_padded(token_batch, batch_pad=0):
    maxlen = max(map(lambda x: len(x), token_batch)) if batch_pad == 0 else batch_pad
    return map(lambda token_list: token_list + [-1] * (maxlen - len(token_list)), token_batch)
...
def refill_batches(batches, word2id, context_file, qn_file, ans_file, batch_size, context_len, question_len, discard_long):
    ...
    while context_line and qn_line and ans_line: # while you haven't reached the end
        ...
        if len(qn_char_ids) > 9*question_len:
            if discard_long:
                continue
            else: # truncate
                qn_char_ids = qn_char_ids[:9*question_len]
        ...
        if len(context_char_ids) > 9*context_len:
            if discard_long:
                continue
            else: # truncate
                context_char_ids = context_char_ids[:9*context_len]
